            ...
            <div class="card shadow-lg border-0 rounded-4">
                <div class="card-body p-4">
                    <div class="text-center mb-4">
                        ...
                        <h3 class="fw-bold mb-1">{{ $lang == 'ar' ? 'إضافة منتج للطلب' : 'Add Product to Order' }}</h3>
                        <p class="text-muted mb-0">{{ $lang == 'ar' ? 'اختر المنتج وأدخل الكمية والسعر.' : 'Select the product, enter quantity and price.' }}</p>
                    </div>
                    <form action="{{ route('pro_order_items.store', $orderId) }}" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <select name="inds_product_id" id="inds_product_id" class="form-select rounded-pill" required>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="number" name="price" id="price" required class="form-control rounded-pill" readonly>
                        </div>
                        <div class="mb-4">
                            <label class="form-label fw-semibold"><i class="fas fa-calculator text-success me-1"></i> {{ $lang == 'ar' ? 'الإجمالي' : 'Total' }}</label>
                            <input type="text" id="total" class="form-control rounded-pill bg-light" readonly>
                        </div>
                        <div class="d-grid">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var select = document.getElementById('inds_product_id');
