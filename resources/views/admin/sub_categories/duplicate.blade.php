        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">
                        <i class="fas fa-copy"></i> تكرار قسم فرعي
                    </h3>
                ...
                <div class="card-body">
                    <div class="alert alert-info">
                        ...
                        سيتم نسخ القسم الفرعي "{{ $subCategory->name_ar }}" مع جميع الحقول المرتبطة به.
                    </div>

                    <form action="{{ route('admin.sub_categories.duplicate-store', $subCategory->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name_ar" id="name_ar" class="form-control @error('name_ar') is-invalid @enderror"
                                           value="{{ old('name_ar', $subCategory->name_ar . ' - نسخة') }}" required>
                                    @error('name_ar')
                            ...
                            <div class="col-md-6">
                                <div class="form-group">
                                    ...
                                    <input type="text" name="name_en" id="name_en" class="form-control @error('name_en') is-invalid @enderror"
                                           value="{{ old('name_en', $subCategory->name_en . ' - Copy') }}" required>
                                    @error('name_en')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group">
                                    ...
                                    @if($subCategory->image)
                                        <div class="mb-2">
                                            ...
                                            <small class="form-text text-muted">الصورة الحالية ستتم نسخها تلقائياً</small>
                                        </div>
                                    @endif
                                    ...
                                    @error('image')
                                        <div class="invalid-feedback">{{ $message }}</div>
                                    @enderror
                                    <small class="form-text text-muted">إذا لم ترفع صورة جديدة، سيتم نسخ الصورة الحالية</small>
                                </div>
                            </div>
                        </div>
                        ...
                        @php
                            $fieldsCount = $subCategory->fields ? $subCategory->fields->count() : 0;
                        @endphp
                        @if($fieldsCount > 0)
                            <div class="alert alert-warning">
                                <i class="fas fa-exclamation-triangle"></i>
                                سيتم نسخ {{ $fieldsCount }} حقل مرتبط بهذا القسم الفرعي.
                            </div>
                        @endif
