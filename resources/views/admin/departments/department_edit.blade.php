    ...
    <div class="col-md-12">
        ...
        <form action="" id="form-category" method="post" enctype="multipart/form-data"> @csrf
                            ...
                            @if ($parent->sub_Departments->count() > 0)
                                @foreach ($parent->sub_Departments as $subDepartment)
                                    <option value="{{ $subDepartment->id }}"
                                        {{ selected(isset($department) && $department->department_id, $subDepartment->id) }}>
                                        &nbsp;&nbsp;&nbsp;&raquo; {{ $lang == 'ar' ? $subDepartment->name_ar : $subDepartment->name_en }}
                                    </option>
                                @endforeach
                            @endif
                        @endforeach
            ...
            <div class="form-group row {{ $errors->has('topics') ? ' has-error' : '' }}">
                <label class="mb-3 col-sm-3 control-label">{{ __('category.categories') }} <span style="color:red;">*</span></label>
                <div class="col-sm-7">
