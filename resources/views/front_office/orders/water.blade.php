...
@section('content')
    ...
    <?php $offer = $service->comments->where('service_provider', $order->service_provider_id)->first();
            $rating_water = App\Models\Rating::where('order_id' , $order->id)->where('department_name' , 'water')->first(); 
    ?>
                                    ...
                                    <div>
                                        @if ($user->id == $order->customer_id)
                                            @if ($order->status == 'pending')
                                                <a href="{{ route('accept_project_water', $order->id) }}"
                                                    class="btn btn-primary">{{ ($lang == 'ar') ? 'أستلام المشروع' : 'Confirm Project' }}</a>
                                            @elseif($order->status == 'completed' && !$rating_water)
